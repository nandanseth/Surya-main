...
const UploadModal = ({policyNum, setOpenUpload}) => {
    ...
    const onSubmit = async() => {
        ...
        console.log(uploadDoc.name, uploadDoc)
        ...
        for (let i = 0; i < uploadDoc.length; i++) {
            ...
            onSuccess: (result) => {console.log(result);fileLinks.push(result.url());},
            onError: (error) => console.log(error),
            });
            
        }

        if (!policyJSON.Uploads[uploadName]) {
            policyJSON.Uploads[uploadName] = [];
        }
        policyJSON.Uploads[uploadName].push(...fileLinks);
