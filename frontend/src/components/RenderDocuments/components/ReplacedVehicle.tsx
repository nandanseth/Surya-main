...
function ReplacedVehicle({ policy, keyOne, oldValue, newValue, keys, endDate, endNumber}) {
    ...
    const createMappingObject = (keys, oldValue, newValue) => {
        ...
        keys.forEach((key, index) => {
            ...
            if (Array.isArray(oldValue)) {
                ...
                mapping[num].oldValues.push(oldValue);
            }

            if (Array.isArray(newValue)) {
                mapping[num].newValues.push(newValue[index]);
            } else {
                mapping[num].newValues.push(newValue);
            }
            
        });
        console.log(mapping, 'deer')
        return mapping;
    }

    const getVehicleChangesFromSlice = () => {
        const oldValues = [];
        const newValues = [];
        ...
        Object.keys(mappingObject).forEach((key) => {
            const oldVehicle = {...policyJSON.vehicles.values[key]}
            const newVehicle = {...policyJSON.vehicles.values[key]}
            
            for (const i in mappingObject[key]['keys']) {
                
                oldVehicle[mappingObject[key]['keys'][i]] = mappingObject[key]['oldValues'][i]
                newVehicle[mappingObject[key]['keys'][i]] = mappingObject[key]['newValues'][i]
            }
            oldValues.push(oldVehicle)
            newValues.push(newVehicle)
        })
        ...
        <Document>
            <Page style={styles.body}>
                <View>
                    ...
                    {getVehicleChangesFromSlice()[1].map((vehicle, index) => {
                        return (
                    ...
                    {getVehicleChangesFromSlice()[0].map((vehicle, index) => {
                    return (
                    <View style={[styles.row]} wrap={false}>
                        <Text style={[styles.cellRomanSmall]}>
                           <>DELETE</>
...
export default ReplacedVehicle
