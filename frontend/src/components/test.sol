...
//  LoopKings 2022-01-26 @futurehelp
...
contract Memes is ERC721URIStorage {
    uint256 tokenId = 1;
    ...
    function buyNFT(uint256 _tokenId) public payable {
        ...
        require(
            ...
            "Price must be equal to the price of the NFT."
        );
        require(
            nfts[_tokenId].owner != msg.sender,
            "Wanna buy your own NFT? Get another account..."
        );
        //_transfer(nfts[_tokenId].owner, msg.sender, _tokenId);
        //payable(nfts[_tokenId].owner).transfer(msg.value);
        safeTransferFrom(nfts[_tokenId].owner, msg.sender, _tokenId);
    ...
    function markNFTForSale(uint256 _tokenId, uint256 _price) public {
        ...
        require(
            nfts[_tokenId].owner == msg.sender,
            "NFT is now ready for sell."
        );
        nfts[_tokenId].price = _price;
        nfts[_tokenId].isForSale = true;
        emit Approval(msg.sender, address(this), _tokenId);
    }

    function transferNFT(address _to, uint256 _tokenId) public {
        require(
            nfts[_tokenId].owner == msg.sender,
            "NFT does not exist or you dont have permission to transfer."
        );
