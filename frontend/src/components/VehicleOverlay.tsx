...
const VehicleOverlay = ({ options = {}, mainInfo = '', subInfo = '', sideInfo = '', close, show }) => (
          ...
          <StyledDiv>
            ...
            <Items>
              {items.map(({ title, info }, i) => (
                <VehicleInfo info={info} key={`${title} + ${i}`} title={title} />
              ))}
            </Items>
          </StyledDiv>
          <Close onClick={close} style={{ alignSelf: 'flex-end', marginTop: 'auto' }}>
            Close
          </Close>
        </Main>
      </Container>
