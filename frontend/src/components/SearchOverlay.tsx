  ...
  return (
    <Overlay>
      <SearchContainer>
        <Title>{title}</Title>
        <MaxDiv>
          <Input label={label} placeholder="Insured" value={insured} onChange={onChange} />
          {insured !== ''
            ? (
              <FilterHolder>
                {
                  insuredList.filter((item) => item
                    .toLower()
                    .contains(insured)).map((item) => (
                      <Item key={item}>
                        <ItemName />
                        <ItemCity />
                        <ItemZip />
...
const Input = ({
  onChange, style, label, value, placeholder,
} :
{
  onChange?: any;
  style?: any;
  ...
  <Div>
    <Label>{label}</Label>
    <StyledInput type="text" onChange={onChange} value={value} style={style} placeholder={placeholder || (`${label}`)} />
  </Div>
);
