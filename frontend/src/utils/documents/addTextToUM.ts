...
import Camera from '../../components/RenderDocuments/components/PDFs/Camera.pdf'
import Delay from '../../components/RenderDocuments/components/PDFs/DELAYED_REPORTING.pdf'
...
export const addTextToPDF = async (texts, states, CSLOrSplit) => {
    ...
    const tertiaryFormMapping = {
        'Arizona': {'Combined Single Limit': 6, 'Split Limit': 6},
        'Pennsylvania': {'Combined Single Limit': 7, 'Split Limit': 5}
    }
    ...
    for (let i = 0; i < pages.length; i++) {
        ...
        for (let j = 0; j < positions.length; j++) {
            ...
            if (secondaryFormStates.includes(states)) {
                if (tertiaryFormInts.includes(j) && i===tertiaryFormMapping[states][CSLOrSplit]) {
                    const yNew = y-20
                    page.drawText(texts[j], {x, y: yNew, size: 11});
                }
            }
            ...
            console.log(secondaryFormStates.includes(states),pages.length,'dofm')
...
export const addTextToCameraPDF = async (texts) => {
    ...
    const pdfPath = '../../components/RenderDocuments/components/PDFs/Camera.pdf'
...
export const addTextToDelayPDF = async (texts) => {
    ...
    const pdfPath = '../../components/RenderDocuments/components/PDFs/DELAYED_REPORTING.pdf'
    ...
    for (let i = 0; i < pages.length; i++) {
        ...
        for (let j = 0; j < positions.length; j++) {
            ...
            if (i === 1 && j === 0) {
                page.drawText(texts[j], {x: 140, y: 602.5, size: 11});
            }

            if (i === 1 && j === 1) {
                page.drawText(texts[j], {x: 155, y: 627.5, size: 11});
            }
