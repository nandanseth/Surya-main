...
const getDataFromList = (vinList:[]) => {
  const setValues = new Set(['Make', 'Manufacturer Name',
    'Model', 'Model Year', 'Series', 'Trim', 'Vehicle Type', 'Body Class',
    'Number of Seats']);
...
const getInoFromVin = async (vin: string) => {
  try {
    const { data: { Results } } = await axios.get(`https://vpic.nhtsa.dot.gov/api/vehicles/decodevin/${vin}?format=json`);
